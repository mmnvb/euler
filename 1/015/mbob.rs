...
fn main(){
    // this is the size of the grid (since we have equal sizes let's use 1 variable only)
    let a = 20;

    let steps = a+a; // total number of step it takes to reach the right bottom in any way (since we have 21x21 points)

    // now we need to answer the question
    // in the case of 3x3 grid
    // we have 6 places and 3 ways for each direction. How many combinations are there considering that we don't want any duplications (order doesn't matters)
